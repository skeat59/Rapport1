...
else
{
	...
	\section{Son}
		...
		Pour naviguer dans le jeux nous avons mis différents "États". Ces "État" sont les pages de menus, de selection du mode, les pages de jeux, etc.. 
	...
	switch (CurrentGameState)
	{
		...
		Avec cette configuration, chaque état a sa propre Classe. Le Game1 se retrouve donc largement simplifier puisque qu'avec le switch il sait quel État actualiser ou afficher.\\
	...
	\noindent Screen de l'editeur : 
		...
		\newpage
			...
			Nos Maps sont basées sur des fichiers textes (en .txt) que nous renommons en .lvl pour level (sans blagues). Le fichier est composé de:\\
						...
						\indent- une première ligne indiquant le nombre de lignes de bloc ou seront placés des \\\indent obstacles ou des bonbon.\\
						
						\indent- une deuxième ligne indiquant cette fois le nombre d'élément à afficher sur chaque\\\indent ligne. C'est donc l'affichage horizontale du jeu.\\
			 ...
			 \indent Lors de la création de l'éditeur de map nous avons eu différents problèmes. Tout d'abord lors de la création des éléments visuels tels que les boutons, n'ayant pas de graphiste dans le groupe, cette tache nous a pris plus de temps que prévu. Au niveau de la création de la partie fonctionnel de l'éditeur de map nous ne savions pas du tout comment nous y prendre. Puis, après avoir étudier nos besoins et ceux des utilisateur, nous avons pu mettre en place les différentes parties de notre éditeur. Skeat et Vincae se sont partagés le travail.
			 
			 Vincae s'est occupé de toute la partie affichage des différents obstacles et bonbons afin de facilité la vision d'ensemble d'un niveaux ainsi que du chargement de map existante et de la sauvegarde des maps.\\
	...
	\end{mylisting}
		...
		Dans cette partie du programmes qui permet la sauvegarde d'une carte a partir de  notre éditeur, j'utilise une deux boucles imbriquées. La première gère les lignes tandis que la deuxième correspond à une place dans cette ligne (ce qui correspond donc aux colonnes).
		
		De cette manière on parcours la map dans sa totalité et pour chaque élément on va écrire dans le fichier .lvl la valeur du bloc lui correspondant contenu dans le tableau layer. 
		...
		Tandis que Skeat s'est occupé de la modification en temps réel de la map. Cette partie consiste donc à récupérer le bloc que l'utilisateur sélectionne, et le placer à l'endroit souhaité sur la map. Une petite difficulté s'est posée sur cette partie puisqu'il est question de savoir ou le clic à été effectué pour placer le bloc au bon endroit dans la matrice de la map puis l'afficher sous la souris, tous ca de façon optimisée pour ne pas faire ramer l'éditeur.\\
	...
	\section{Réseau}
	Comme expliqué dans l'introduction, notre jeux permet, si l'on se connecte au démarrage du jeux, d'envoyer le score à la base de données de notre site web.\\
	
	Avec quelques tutos en ligne et les connaissances de T4ze dans le domaine du Web, la compréhension et l'implémentation des fonctions nécessaire fut assez simple et fonctionnel rapidement.
	
	La gestion de ce qui concerne la base de donnée (lecture, écriture, test) utilise une Dll nommée "Mysql.Data" car il n'est, par défaut, pas possible de faire communiquer un environnement de jeux en C\# avec une base de données en MySql. Cette Dll est disponible en téléchargement libre sur internet et se trouve très simplement parfois même avec des exemples de fonctions. Mais nous avons préférer tout reprendre nous même pour avoir un code propre, optimisé et surtout que nous contrôlons entièrement.
	
	Pour comprendre et pouvoir la contrôler sans faire planter notre jeux, nous avions préalablement implémenté la classe DBConnect.cs dans un projet annexe. Nous avons ainsi pu tester les fonctions d'ajout de données, de suppressions, de mises à jour, de tests d'existences de valeur et de listages au fur et à mesure qu'elles étaient écrites. Certaines fonctions crées ne nous sont pas utile dans l'état actuel du jeux. Mais puisque nous étions sur notre lancée et dans une pensée d'améliorations possibles du jeux si l'on veux par la suite, par exemple, rajouter des options tel que s'inscrire depuis le jeux, afficher le classement ou encore changer de pseudo, elles seront déjà prête.\\
	...
	Comme vous pouvez le voir sur le screen de la base de données, pour plus de sécurité nous chiffrons votre mot de pass en md5. De cette façon, même si un individu parvenait à s'infiltrer dans notre site (quasiment impossible vu notre sécurité), il ne pourrais pas modifier votre score, vous êtes donc bien protéger chez nous. C\# contient une méthode de chiffrement md5 qui nous facilite grandement le test de correspondance de mot de pass puisqu'à notre niveau implémenter ce chiffrement aurait été vraiment compliqué.
	
		
	\newpage
	
	Toute cette partie nous sers à ceci : Le classement inter-joueur
	...
	L'envoie du score se fait uniquement en mode Map Infini (le score ne se base que sur le nombre de mètres parcourut, il n'y a pas donc pas de score comptabilisé sur les maps prédéfinies). Cette envoie est réalisé en fin de partie, lorsque vous avez perdu et uniquement si vous vous étiez au préalable connecté à votre compte CandyBird. Cette enregistrement est obligatoire car sans lui vous n'existez pas dans la base de donnée et donc votre score n'est pas comptabilisé dans le classement.
	...
	\section{Graphique}
	La partie graphique du projet a été compliqué suite au départ imprévu d'Adrien. C'était lui qui était le plus expérimenté face aux logiciels d'éditions d'images (Photoshop en l'occurrence). Il donc très vite fallu apprendre les bases de ce logiciel afin de pouvoir réutiliser et modifier les travaux qu'avais déjà produit Adrien. Ce petit problème n'était pas des moindre puisque un jeu avec des graphismes mal finis n'ai pas très attirant à jouer et nous le savons bien.
	...
	\indent Nous avons tout de même défini suivi un modèle de départ pour partir sur les mêmes bases afin de ne pas se retrouver avec des thèmes décalé d'une partie à l'autre. Chaque modification est montrée à tout les membres de l'équipe pour qu'ils puissent donner leur avis et valider ou non.
	
	\vspace{15mm}
	
	\subsection {Sprite}
	Le personnage principal n'étant pas tellement d'accord avec la façon dont l'on se sert de lui dans le jeux, bien qu'il ai fermé les yeux sur cette utilisation, il à préféré rester quelque peu dans l'ombre (ce qui explique sont allure étonnamment sombre et ses yeux fermés).
